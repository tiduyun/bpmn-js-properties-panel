...
function triggerChangeEvent (e, id, text) {
  let input = e.parentNode.children[0].children[0];
  let refAttr = input.getAttribute('refattr')
  let modelProperty = input.getAttribute('name')

  input.value = JSON.stringify({[modelProperty]: text, [refAttr]: id})
...
module.exports.triggerChangeEvent = triggerChangeEvent
