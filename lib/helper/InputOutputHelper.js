...
/**
 * Checks whether a parameter value (without definition) is a constant value,
 ...
 * @return {string}
 */
InputOutputHelper.checkParameterValue = function(value) {

  // constant values do not have expression clauses ${} and new lines
  if (!this.isExpression(value) && !hasNewLines(value)) {
    return 'constant-value';
  } else {

    // variable expressions have
    ...
    // expressions have expression clauses ${} and are no variable expressions
    return 'expression';
  }
};
...
InputOutputHelper.getParameterType = function(bo) {
  ...
  // (2) use forced type if available
  if (bo.$lastType) {
    return parameterTypes[bo.$lastType];
  }

  // (3) set <variable> as default for null values
  if (!value) {
    return parameterTypes['variable'];
  }

  // (4) check whether value is <constant-value>, <variable> or <expression>
  return parameterTypes[this.checkParameterValue(value)];
};
