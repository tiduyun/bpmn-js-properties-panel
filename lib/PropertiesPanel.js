...
PropertiesPanel.prototype.activateTab = function(tab) {
  ...
  var tabSelected = reduce(allTabNodes, function(selected, tabNode) {
    var currentTabId = domAttr(tabNode, 'data-tab');

    var isSelectedTab = tabId === currentTabId;

    domClasses(tabNode).toggle('bpp-active', isSelectedTab);

    return selected || isSelectedTab;
  }, false);

  if (!tabSelected) {
    domClasses(allTabNodes[ 0 ]).toggle('bpp-active', true);
  }
  ...
  if (!tabSelected) {
    domClasses(allTabLinkNodes[ 0 ]).toggle('bpp-active', true);
  }
};
...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  // TODO: Why the hell is this done in the function that updates visibility?
