...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  forEach(tabs, function(tab, tabIndex) {
    ...
    forEach(groups, function(group) {
      ...
      var closeGroup = function(evt) {
        domClasses(groupNode).toggle('group-closed');
        evt.preventDefault();
        evt.stopPropagation();
      };

      groupNode.querySelector('.group-toggle').addEventListener('click', closeGroup);
      groupNode.querySelector('.group-label').addEventListener('click', closeGroup);
