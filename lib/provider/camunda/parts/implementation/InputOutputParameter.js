...
module.exports = function(element, bpmnFactory, options, translate) {
  ...
  entries.push(entryFactory.selectBox({
    ...
    get: function(element, node) {
      ...
      if (bo && !bo.$lastType) {
        bo.$lastType = parameterType.value;
      }
    ...
    set: function(element, values, node) {
      ...
      if (parameterType === 'script' || parameterType === 'list' || parameterType === 'map') {
        ...
        properties.value = bo.$oldValue || bo.value;
        delete bo.$oldValue;
      }
  ...
  entries.push(entryFactory.validationAwareTextField({
    ...
    validate: function(element, values, node) {
      ...
      var validation = {},
          parameterType = inputOutputHelper.checkParameterValue(value);

      if (parameterType !== 'variable') {
        var validationText =
          translate('Value is not a valid variable. Please change type to "' +
          inputOutputHelper.getAllParameterTypes(bo)[parameterType].label +
          '".');
  ...
  entries.push(entryFactory.validationAwareTextField({
    ...
    validate: function(element, values, node) {
      ...
      if (!value || (bo || {}).$lastType !== 'constant-value') {
        return;
      }
      ...
      if (parameterType !== 'constant-value') {
        var validationText =
          translate('Value is not a valid constant value. Please change type to "' +
          inputOutputHelper.getAllParameterTypes(bo)[parameterType].label +
          '"');
        validation.value = validationText;
        bo.$oldValue = value;
  ...
  '</div>',
    ...
    validate: function(element, values, node) {
      ...
      if (!value || (bo || {}).$lastType !== 'expression') {
        return;
      }
      ...
      if (parameterType === 'constant-value') {
        validationText = translate('Value is identified as constant value. Please change type to "Constant Value"');
        validation.value = validationText;
        bo.$oldValue = value;
      } else if (parameterType === 'variable') {
        validationText =
          translate('Value is identified as variable. Please change type to "' +
          inputOutputHelper.getAllParameterTypes(bo)[parameterType].label +
          '"');
        validation.value = validationText;
        bo.$oldValue = value;
