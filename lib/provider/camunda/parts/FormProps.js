...
module.exports = function(group, element, bpmnFactory, translate) {
  ...
  group.entries.push(entryFactory.comboBox({
    ...
    selectOptions: [
      ...
      // { name: '租户选择', value: 'tenantSelect' },
      // { name: '项目选择', value: 'projectSelect' },
      // { name: '资源申请复合组件', value: 'resApplyComponent' },
      // { name: '资源变更复合组件', value: 'resChangeComponen' },
      // { name: '服务目录工单复合组件', value: 'instanceTypeComponen' },
      { name: '附件上传', value: 'file' }
    ],
    modelProperty: 'type',
    emptyParameter: true,
    ...
    set: function(element, values, node) {
      ...
      if (selectedFormField.type === 'select' && values.type !== 'select') {
        // delete camunda:value objects from formField.values when switching from type select
        commands.push(cmdHelper.updateBusinessObject(element, selectedFormField, { values: undefined }));
      }
  ...
  group.entries.push(entryFactory.comboBox({
    id: 'form-field-selectValue',
    label: '下拉框类型',
    selectOptions: [
      { name: 'company', value: 'company' },
      { name: 'project', value: 'project' }
    ],
    modelProperty: 'selectValue',
    // emptyParameter: true,
    // divider: true,
    get: function(element, node) {
      var selectedFormField = getSelectedFormField(element, node);
      ...
      if (selectedFormField) {
        return { selectValue: selectedFormField.$attrs.selectValue };
      } else {
        return {};
      }
    },
    ...
    hidden: function(element, node) {
      var selectedFormField = getSelectedFormField(element, node);
      return !selectedFormField || (selectedFormField && selectedFormField.type !== 'select')
    }
  }))
