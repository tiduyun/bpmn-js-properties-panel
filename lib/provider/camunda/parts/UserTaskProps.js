  ...
  if (is(element, 'camunda:Assignable')) {
    ...
    var propSelectOptions = {
      ...
      selectOptions: function(element, inputNode) {
        var selectOptions = [{name: '请选择', value: ''}, { 'name': '审核', value: 'audit' }, { 'name': '评估', value: 'access' }]
      ...
      set: function(element, values, node) {
        ...
        if (!property) {
          property = elementHelper.createElement('camunda:Property', values, properties, bpmnFactory);
          commands.push(cmdHelper.addElementsTolist(element, properties, 'values', [ property ]));
        } else {
          commands.push(cmdHelper.updateBusinessObject(element, property, values))
        }
